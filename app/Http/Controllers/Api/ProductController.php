...
class ProductController extends Controller
{
    ...
    public function index(Request $request, $slug = null)
    {
        ...
        //  If slug is a build-product alias, add custom filter
        $buildProductSlugs = ['engagement-rings'];
        if (in_array($slug, $buildProductSlugs)) {
        ...
        if (!empty($filters['subcategory']) && !empty($filters['category'])) {
            // Case 1: Both category and subcategory are present
            $collectionData = ProductCollection::where('product_category_id', $filters['subcategory'])->get();
        } elseif (!empty($filters['category'])) {
            // Case 2: Only category is present
            $collectionData = ProductCollection::where('parent_category_id', $filters['category'])->get();
        } else {
            // Case 3: No category or subcategory
            // Get one row per group where parent_category_id is not null and product_category_id is null
        ...
        $variationQuery->whereHas('product', function ($query) use ($filters) {
            ...
            if (!empty($filters['is_build_product'])) {
        ...
        if ($isDateSort) {
            ...
            //  Sort by variation price MIN/MAX
            $subQuery = ProductVariation::select('product_id', DB::raw("$priceAggFunc(price) as target_price"))
                ->whereIn('product_id', $filteredProductIds)
                ->groupBy('product_id');

            // $totalProducts = ProductVariation::joinSub($subQuery, 'sorted_prices', function ($join) {
        ...
        foreach ($products as $product) {
            ...
            if ($product->is_build_product == 1) {
                ...
                // Optional sorting by metal quality
                // uksort($groupedByMetal, function ($a, $b) use ($variations) {
                //     $getQuality = function ($metalId) use ($variations) {
                //         return (int) optional($variations->firstWhere('metal_color_id', $metalId)?->metalColor)->dmt_tooltip ?? 999;
                //     };
                //     return $getQuality($a) <=> $getQuality($b);
                // });
            } else {
                //  Default existing format: metal_color_id => [variations...]
    ...
    // {
    //     // Step 1: Find the product by master_sku
    ...
    //     // Step 2: Group variations by metal color
    //     $variations = $product->variations;
 
    //     $groupedByMetal = $variations->groupBy('metal_color_id')->map(function ($group) use ($product) {
    //         return $group->map(function ($variation) use ($product) {
    //             $category = $product->productcategory;
    //             $parent = $category?->parent;
    ...
    //     // Sort metal_color groups by quality (like before)
    //     $groupedByMetal = collect($groupedByMetal)->sortBy(function ($variations, $key) {
    ...
    //             'name' => $product->productcategory->category_name,
    //             'parent' => $product->productcategory->parent ? [
    //                 'id' => $product->productcategory->parent->category_id,
    //                 'name' => $product->productcategory->parent->category_name,
    //             ] : null
    //         ] : null,
    //         'metal_variations' => $groupedByMetal,
    //     ]);
